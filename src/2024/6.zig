...
fn part1(input: InputType) ReturnType {
    ...
    while (true) {
        ...
        if (row_new < 0 or row_new >= input.tiles.len or col_new < 0 or col_new > input.tiles[@intCast(row)].len)
            break
        else if (input.tiles[@intCast(row_new)][@intCast(col_new)] == Tile.Obstacle) {
            if (col_step == 0) {
                col_step = -row_step;
                row_step = 0;
            } else {
